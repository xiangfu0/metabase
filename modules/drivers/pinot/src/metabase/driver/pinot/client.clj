...
(defn details->url
  "Helper for building a Pinot URL.
   ...
   (details->url {:host \"http://localhost\", :port 9000}) -> \"http://localhost:9000\""
  [{:keys [host port]} & strs]
  ...
  (let [{:keys [auth-enabled auth-username auth-token-value]} options
    ...
    (try
      (let [{:keys [status body]} (request-fn url options)]
        ;;(log/infof "Pinot request url: %s, body: %s" url body)
        (when (not= status 200)
  ...
  (ssh/with-ssh-tunnel [details-with-tunnel details]
    (try
      ...
      (let [url (details->url details-with-tunnel "/sql")
        ...
        (log/infof "Pinot query: %s" query)
        (log/infof "Pinot details: %s" details)
        ;;(log/infof "Parsed Pinot response: %s" response)
