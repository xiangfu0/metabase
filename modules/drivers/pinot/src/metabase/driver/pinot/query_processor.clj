  ...
  (let [filters (:filter original-query)]
    (letfn [(filter-clause [filter]
              (if (vector? filter)
                (let [[op & args] filter]
                  (case op
                    "and" (str/join " AND " (map filter-clause args))
                    "or" (str/join " OR " (map filter-clause args))
                    "is-null" (str (resolve-field (first args)) " IS NULL")
                    "is-not-null" (str (resolve-field (first args)) " IS NOT NULL")
                    (let [[field value] args
                          field-name (resolve-field field)
                          value-str (if (map? value)
                                      (:name value)
                                      (if (string? value) (str "'" value "'") value))]
                      (str field-name " " (name op) " " value-str))))
                (throw (ex-info "Invalid filter format" {:filter filter}))))]
      (if filters
        (assoc-in pinot-query [:query :where] (filter-clause filters))
        pinot-query))))
...
(defn- handle-aggregations
  ...
  (let [aggregations (:aggregation original-query)]
    (log/infof "Aggregation data: %s" aggregations)
    (let [aggregation-clauses (map (fn [aggregation]
                                     (let [[_ agg-type options] aggregation
                                       ...
                                       (log/infof "Processing aggregation clause: %s, agg-type: %s, agg-fields: %s" aggregation, agg-type, agg-fields)
                                       (let [result (cond
                                         ...
                                         (log/infof "Generated aggregation clause: %s" result)
                                         result)))
                                   aggregations)]
...
(defn- generate-pinot-sql
  ...
  (let [
        ...
        limit-clause (if-let [limit (get-in pinot-query [:query :limit])]
                       (str "LIMIT " limit)
                       nil)
        offset-clause (if-let [offset (get-in pinot-query [:query :offset])]
                        (str "OFFSET " offset)
                        nil)
        clauses (remove nil? [select-clause from-clause where-clause group-by-clause order-by-clause limit-clause offset-clause])]
