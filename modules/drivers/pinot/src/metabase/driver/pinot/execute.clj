...
(defn- post-process
  [results]
  (log/info "Raw results from Pinot: " results) ;; Log the raw results
      ...
      (try
        (let [processed-results {:projections column-names  ;; Use inferred projections from the schema
                                  :results     (map (fn [row]
                                                    ...
                                                    rows)}]
          ;; (log/info "Processed results: " processed-results) ;; Log the processed results
          processed-results)
        (catch Throwable e
...
(defn- result-rows [{rows :results, :as results} actual-col-names annotate-col-names]
  ...
  (log/info "actual-col-names:" actual-col-names)
  (log/info "annotate-col-names:" annotate-col-names)
  (log/info "Rows (first 5):" (take 5 rows))
                                ...
                                (if (contains? row col-name)
                                  ...
                                  (throw (ex-info (tru "Column {0} not found in row" col-name)
                                                  {:col-name col-name
                                                   :row row})))))
                            actual-col-names))]
      ;; Add logging for the getters
      (log/info "Generated getters:" getters)

      ;; If no getters are found, throw an error
      (when (empty? getters)
        (throw (ex-info (tru "Dont know how to retrieve results for columns {0}" (pr-str actual-col-names))
                        {:type    qp.error-type/driver
                         :results results})))
  ...
  (let [col-names (if mbql?
        ...
        updated-metadata (update metadata :cols (fn [cols]
                                                  (map (fn [col base-type]
                                                       ...
                                                       cols base-types)))]
    (log/info "Responding with metadata and rows:" updated-metadata rows) ; Log for debugging
